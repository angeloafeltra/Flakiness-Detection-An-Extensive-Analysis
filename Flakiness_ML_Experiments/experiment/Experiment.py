...
class Experiment(ClassicExperiment):
    ...
    def run(self,X_train_set,y_train_set,X_test_set,y_test_set):
        ...
        y_predict=self.pipeline.predict(X=X_train_set)
        super().setPerformanceTrain(y_train_set,y_predict)
        '''
        self.setAccuracyTrain(accuracy_score(y_train_set,y_predict))
        self.setPrecisionTrain(precision_score(y_train_set,y_predict))
        self.setRecallTrain(recall_score(y_train_set,y_predict))
        self.setF1Train(f1_score(y_train_set,y_predict))
        self.setAUCTrain(roc_auc_score(y_train_set, y_predict))
        tn, fp, fn, tp = confusion_matrix(y_train_set, y_predict).ravel()
        self.setTP_Train(tp)
        self.setFP_Train(fp)
        self.setFN_Train(fn)
        self.setTN_Train(tn)
        self.setMCC_Train(matthews_corrcoef(y_train_set, y_predict))
        '''

        y_predict=self.pipeline.predict(X=X_test_set)
        super().setPerformance(y_test_set,y_predict)
        '''
        self.setAccuracy(accuracy_score(y_test_set,y_predict))
        self.setPrecision(precision_score(y_test_set,y_predict))
        self.setRecall(recall_score(y_test_set,y_predict))
        self.setF1(f1_score(y_test_set,y_predict))
        self.setAUC(roc_auc_score(y_test_set, y_predict))
        tn, fp, fn, tp = confusion_matrix(y_test_set, y_predict).ravel()
        self.setTP(tp)
        self.setFP(fp)
        self.setFN(fn)
        self.setTN(tn)
        self.setMCC(matthews_corrcoef(y_test_set, y_predict))
        '''
